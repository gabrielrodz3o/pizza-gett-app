...
function AddressEditor({ address, first, onDone, onCancel }: {
  ...
  const save = useMutation({
    ...
    onSuccess: () => { haptics.success(); onDone(); },
    onError: (error: any) => { haptics.error(); confirmSheet({ title: 'No se pudo guardar', message: error?.response?.data?.message || error.message || 'Revisa la ubicación.', icon: 'alert-circle', confirmText: 'Entendido', cancelText: null }); },
  });
  ...
  const useMyLocation = async () => {
    ...
    try {
      ...
      if (!permission.granted) return confirmSheet({ title: 'Permiso necesario', message: 'Permite el acceso a tu ubicación para colocar el pin.', icon: 'locate', confirmText: 'Entendido', cancelText: null });
...
export function AddressesModal({ visible, onClose }: Props) {
  ...
  const remove = useMutation({ mutationFn: deleteAddress, onSuccess: () => { haptics.success(); qc.invalidateQueries({ queryKey:['customer-home'] }); }, onError: () => { haptics.error(); confirmSheet({ title: 'No se pudo eliminar', message: 'No se pudo eliminar la dirección. Intenta nuevamente.', icon: 'alert-circle', confirmText: 'Entendido', cancelText: null }); } });
...
const s = StyleSheet.create({
  screen:{ flex:1,backgroundColor:colors.cream },header:{ flexDirection:'row',alignItems:'center',justifyContent:'space-between',paddingHorizontal:20,paddingVertical:14,borderBottomWidth:1,borderBottomColor:colors.border,backgroundColor:'white' },headerTitle:{ fontSize:17,fontFamily: font.black,color:colors.text },content:{ padding:16,paddingBottom:40 },
  addressCard:{ backgroundColor:'white',borderRadius:16,padding:14,flexDirection:'row',alignItems:'center',gap:10,marginBottom:10,borderWidth:1,borderColor:colors.border },addressIcon:{ width:40,height:40,borderRadius:12,backgroundColor:colors.yellowSoft,alignItems:'center',justifyContent:'center' },addressTopRow:{ flexDirection:'row',alignItems:'center',gap:8,marginBottom:2 },addressLabel:{ fontSize:14,fontFamily: font.black,color:colors.text },addressStreet:{ fontSize:13,color:colors.text,lineHeight:18 },addressRef:{ fontSize:11,color:colors.muted,marginTop:2 },defaultBadge:{ backgroundColor:colors.yellowSoft,borderRadius:6,paddingHorizontal:6,paddingVertical:2 },defaultText:{ fontSize:10,fontFamily: font.extraBold,color:colors.brown },
  empty:{ alignItems:'center',paddingTop:60,gap:10 },emptyTitle:{ fontSize:18,fontFamily: font.black,color:colors.text },emptyBody:{ fontSize:13,color:colors.muted,textAlign:'center',lineHeight:19 },addBtn:{ flexDirection:'row',alignItems:'center',gap:8,backgroundColor:colors.yellow,borderRadius:14,paddingHorizontal:20,paddingVertical:12,marginTop:10 },addBtnText:{ fontSize:14,fontFamily: font.black,color:colors.brown },addInlineBtn:{ flexDirection:'row',alignItems:'center',gap:8,justifyContent:'center',paddingVertical:16 },addInlineBtnText:{ fontSize:14,fontFamily: font.extraBold,color:colors.orange },
  form:{ backgroundColor:'white',borderRadius:20,padding:14,borderWidth:1,borderColor:colors.border },editorTitleRow:{ flexDirection:'row',justifyContent:'space-between',alignItems:'center',marginBottom:12,gap:8 },editorActions:{ flexDirection:'row',alignItems:'center',gap:7 },formTitle:{ fontSize:18,fontFamily: font.black,color:colors.text },formHint:{ fontSize:12,color:colors.muted,marginTop:2 },gpsBtn:{ width:42,height:42,borderRadius:13,backgroundColor:colors.yellow,alignItems:'center',justifyContent:'center' },topSaveBtn:{ height:42,borderRadius:13,backgroundColor:colors.brown,paddingHorizontal:13,alignItems:'center',justifyContent:'center' },topSaveText:{ color:'white',fontSize:12,fontFamily: font.black },mapWrap:{ height:300,borderRadius:16,overflow:'hidden',backgroundColor:colors.cream },map:{ flex:1 },mapLoader:{ marginTop:130 },apiWarning:{ flexDirection:'row',alignItems:'center',gap:7,backgroundColor:'#FFF4D6',borderRadius:11,padding:10,marginTop:9 },apiWarningText:{ flex:1,fontSize:11,lineHeight:15,color:'#754600',fontFamily: font.bold },coverage:{ flexDirection:'row',gap:8,alignItems:'center',padding:11,borderRadius:12,marginTop:10 },coverageOk:{ backgroundColor:'#E9F8EE' },coverageBad:{ backgroundColor:'#FFF0F0' },coverageTitle:{ fontSize:12,fontFamily: font.extraBold,color:colors.text },coverageSub:{ fontSize:11,color:colors.muted,marginTop:2 },formLabel:{ fontSize:12,fontFamily: font.bold,color:colors.muted,marginBottom:6,marginTop:12 },labelRow:{ flexDirection:'row',gap:8 },labelChip:{ paddingHorizontal:14,paddingVertical:8,borderRadius:10,backgroundColor:colors.cream,borderWidth:1,borderColor:colors.border },labelChipActive:{ backgroundColor:colors.yellow,borderColor:colors.yellow },labelChipText:{ fontSize:13,fontFamily: font.bold,color:colors.muted },labelChipTextActive:{ color:colors.brown },input:{ height:48,borderRadius:12,backgroundColor:colors.cream,borderWidth:1,borderColor:colors.border,paddingHorizontal:14,fontSize:14,color:colors.text },formRow:{ flexDirection:'row',gap:10,marginTop:18 },btn:{ flex:1,height:48,borderRadius:14,alignItems:'center',justifyContent:'center' },btnPrimary:{ backgroundColor:colors.yellow },btnSecondary:{ backgroundColor:colors.cream,borderWidth:1,borderColor:colors.border },btnDisabled:{ opacity:.45 },btnPrimaryText:{ fontSize:14,fontFamily: font.black,color:colors.brown },btnSecondaryText:{ fontSize:14,fontFamily: font.bold,color:colors.muted },
});
